...
import {
  parseCreative,
  soloGeneratePublicRequestSchema,
...
export async function POST(request: Request) {
  try {
    ...
    const body = parseCreative(soloGeneratePublicRequestSchema, await readJamJsonBody(request))
    return NextResponse.json(await getCreativeService().soloGenerate(userId, body), {
