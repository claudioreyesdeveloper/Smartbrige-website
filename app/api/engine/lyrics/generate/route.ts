...
import {
  lyricGeneratePublicRequestSchema,
  parseCreative,
...
export async function POST(request: Request) {
  try {
    ...
    const body = parseCreative(lyricGeneratePublicRequestSchema, await readJamJsonBody(request))
    return NextResponse.json(await getCreativeService().lyricGenerate(userId, body), {
