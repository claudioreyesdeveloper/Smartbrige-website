/**
 * Upsert billing entitlement rows from Stripe Subscription objects.
...
export async function upsertFromSubscription(
  ...
  // Derive plan from the Stripe price id — that's what was actually
  // purchased/changed to, and is authoritative over any client-supplied
  // metadata. If the price id doesn't match a configured plan (stale price,
  // env mismatch), keep whatever plan the row already had rather than
  // guessing; brand-new rows default to style_maker (the historical
  // single-product behaviour, and the schema column default).
  const derivedPlan = planForPriceId(priceId)
  const plan: Plan | undefined =
    derivedPlan ?? (existing[0]?.plan as Plan | undefined) ?? undefined
