...
import type { CapturedMidiEvent } from "@/lib/yamaha/style-capture"
...
/**
 * Port of JamPlayerScreen.cpp styleTrackIndexForYamahaSysEx.
 * SysEx payload is bytes after F0 (and typically through F7), matching
 * JUCE getSysExData() layout for the address bytes we read (0..5).
 *
 * YamahaProtocol: kYamahaId=0x43, kSubIdParamChange=0x10, kModelIdXG=0x4C,
 * kAddrEffect2=0x03 (style blocks 20–27), kAddrMultiPart=0x08 / Ext=0x0A.
 */
export function styleTrackIndexForYamahaSysEx(sysexPayload: number[]): number {
  if (sysexPayload.length < 6) return -1
  const yamahaId = sysexPayload[0]
  const subId = sysexPayload[1]
  const modelId = sysexPayload[2]
  const addrHigh = sysexPayload[3]
  const addrMid = sysexPayload[4]
  if (yamahaId !== 0x43) return -1
  if ((subId & 0xf0) !== 0x10) return -1
  if (modelId !== 0x4c) return -1

  // Effect2 style insertion blocks 20–27 → tracks 0–7
  if (addrHigh === 0x03 && addrMid >= 20 && addrMid <= 27) {
    return addrMid - 20
  }

  // MultiPart / MultiPartExt parts 0x08–0x0f → tracks 0–7
  if ((addrHigh === 0x08 || addrHigh === 0x0a) && addrMid >= 0x08 && addrMid <= 0x0f) {
    return addrMid - 0x08
  }
...
/**
 * Port of JamPlayerScreen::stopSectionRecording filter + renumber.
 *
 * Desktop tooltip / behavior (Include CC / Controllers):
 * - Off (default): keep only note on/off on channels 9–16.
 * - On: also keep CC, program change, pitch wheel, aftertouch, channel
 *   pressure, and SysEx (export later keeps only mapped Yamaha SysEx).
 */
export function filterAndRenumberSectionCapture(
  ...
  for (const event of events) {
    // Desktop: SysEx kept only when includeControlData; unmapped ones drop at export.
    ...
    // Desktop whitelist: note on/off, controller, program change, pitch wheel,
    // aftertouch (poly 0xA0), channel pressure (0xD0).
    const isNoteOnOrOff = kind === 0x80 || kind === 0x90
    const isChannelVoiceMessage =
      isNoteOnOrOff ||
      kind === 0xb0 || // controller
      kind === 0xc0 || // program change
      kind === 0xe0 || // pitch wheel
      kind === 0xa0 || // polyphonic aftertouch
      kind === 0xd0 // channel pressure

    if (!isChannelVoiceMessage) continue

    // Desktop: drop CC/PC/pitch/aftertouch/pressure when includeControlData is false.
    if (!includeControlData && !isNoteOnOrOff) continue
...
/**
 * Wall-clock capture (SmartBridgeContext HiRes / web performance.now) → ticks:
 * PPQ * (BPM / 60). Same as JamPlayerMotifScreen / RiffMaker.
 *
 * Note: JamPlayerScreen_Recording.cpp still has PPQ*bpm/120 with a “Tyros
 * double-rate” comment; that halves ticks against real seconds and plays at
 * double tempo in Cubase. Do not copy that /2 onto web wall-clock takes.
 */
...
function serializeTrackEvents(
  ...
  for (const event of sorted) {
    ...
    body.push(...writeVlq(tick - lastTick))
    if (event.status === 0xf0) {
      // SMF SysEx: F0 + length + payload (payload includes terminating F7 when present).
      body.push(0xf0, ...writeVlq(event.data.length), ...event.data)
    } else {
      body.push(event.status, ...event.data)
    }
    lastTick = tick
  }
...
export function exportRecordedStyleMidi(
  ...
  for (const event of events) {
    // Desktop exportRecordedStyleToTempFile: SysEx → styleTrackIndexForYamahaSysEx.
    if (event.status === 0xf0) {
      const trackIndex = styleTrackIndexForYamahaSysEx(event.data)
      if (trackIndex < 0 || trackIndex >= 8) continue
      const tick = secondsToSectionTicks(event.timeSeconds, bpm, anchor, ppq)
      channelTracks[trackIndex].push({
        tick,
        status: 0xf0,
        data: event.data,
      })
